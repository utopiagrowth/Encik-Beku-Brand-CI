...
├── page-flip.browser.js  StPageFlip 2.0.7, vendored (MIT — page-flip.LICENSE.txt)
...
The turning is [StPageFlip](https://github.com/Nodlik/StPageFlip) 2.0.7, vendored
unchanged as `page-flip.browser.js` (MIT, licence alongside). A first, hand-built
engine was removed after repeated faults; the page code now only builds the 34
page elements, sizes the box the book stretches into, wires the controls and
keeps the page count. Two things carried over from what broke:

- Every page is its own element with its own `<img>`, so a page that has not
  loaded yet is blank paper — it can never show the page it replaced.
- Only the pages around the one in view are fetched: about 700 KB to open the
  book, against 3.1 MB for the PDF. Below 760px the library drops to one page.
