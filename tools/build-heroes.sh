...
# What we do banners, one per service section, named by the section's
# data-tab key - services.html builds the path from that key.
for f in _source/encik-beku-services-hero-*.jpg; do
  sips -Z 1920 -s format jpeg -s formatOptions 80 \
       "$f" --out "$OUT/services-hero-${f##*/encik-beku-services-hero-}" >/dev/null
done
...
printf 'what we do banners:\n'
for f in "$OUT"/services-hero-*.jpg; do
  printf '  %-30s %s\n' "${f##*/}" "$(du -h "$f" | cut -f1)"
done
